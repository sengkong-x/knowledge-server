...
func writeStaleCache(t *testing.T, path string, entries map[string]IndexEntry) {
	t.Helper()
...
func TestLoad_ErrorsOnStaleCacheHeader(t *testing.T) {
	cachePath := filepath.Join(t.TempDir(), "index.gob")
	writeStaleCache(t, cachePath, map[string]IndexEntry{
		"linux/process": {ID: "linux/process", Title: "Process"},
	})
...
func TestLoadOrBuild_FallsBackWhenCacheHeaderStale(t *testing.T) {
	...
	cachePath := filepath.Join(t.TempDir(), "index.gob")
	writeStaleCache(t, cachePath, map[string]IndexEntry{
		"linux/stale": {ID: "linux/stale", Title: "Stale"},
	})
	...
	if _, ok := idx.ByID("linux/process"); !ok {
		t.Fatal("ByID(linux/process) not found, want LoadOrBuild to have rebuilt from the vault")
	}
	if _, ok := idx.ByID("linux/stale"); ok {
		t.Fatal("ByID(linux/stale) found, want the stale cache to have been rejected rather than loaded")
	}
}
